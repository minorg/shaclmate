...
import { BlankNode, Literal, NamedNode, Quad_Graph, Variable } from "@rdfjs/types";
...
function $convertToIdentifierProperty(
  identifier: (() => BlankNode | NamedNode) | BlankNode | NamedNode | string | undefined,
...
function $convertToScalarSet<ItemSourceT, ItemTargetT, Readonly extends boolean>(
...
function $identifierFromRdfResourceValues(
  ...
  options: Parameters<$FromRdfResourceValuesFunction<BlankNode | NamedNode, $IdentifierSchema>>[1],
...
function $intFromRdfResourceValues<T extends number>(
  ...
  return $termLikeFromRdfResourceValues(values, options).chain((values) =>
    values.chainMap((value) => options.schema.in ? value.toInt(options.schema.in) : value.toInt() as Either<Error, T>)
  );
}
...
function $maybeFromRdfResourceValues<ItemT, ItemSchemaT>(
  ...
  return (values, options) =>
    itemFromRdfResourceValues(values, { ...options, schema: options.schema.itemType }).map((values) =>
      values.length > 0
...
function $monkeyPatchObject<T extends object>(
  ...
  if (
    methods.toJson &&
    (!globalThis.Object.prototype.hasOwnProperty.call(obj, "toJSON") || typeof (obj as any).toJSON === "function")
  ) {
  ...
  if (
    methods.$toString &&
    (!globalThis.Object.prototype.hasOwnProperty.call(obj, "toString") || typeof (obj as any).toJSON === "function")
  ) {
...
export namespace $PropertyPath {
  ...
  export const fromRdfResourceValues: $FromRdfResourceValuesFunction<$PropertyPath, object> = (values, options) =>
    values.chainMap((value) => value.toResource().chain((resource) => fromRdfResource(resource, options)));
...
function $stringFromRdfResourceValues<T extends string>(
  ...
  return $termLikeFromRdfResourceValues(values, options).chain((values) =>
    values.chainMap((value) =>
      options.schema.in ? value.toString(options.schema.in) : value.toString() as Either<Error, T>
    )
  );
}
...
const $termLikeFromRdfResourceValues: $FromRdfResourceValuesFunction<
  Resource.Value,
  { readonly hasValues?: readonly (Literal | NamedNode)[]; readonly languageIn?: readonly string[] }
> = (values, { preferredLanguages, schema: { hasValues, languageIn } }) => {
  ...
  if (hasValues && hasValues.length > 0) {
    chain = chain.chain((values) =>
      Either.sequence(hasValues.map((hasValue) => values.find((value) => value.term.equals(hasValue)))).map(() =>
        values
      )
    );
  }
...
export type $ToRdfResourceFunction<ObjectT, IdentifierT extends Resource.Identifier = Resource.Identifier> = (
  object: ObjectT,
  options?: { graph?: Exclude<Quad_Graph, Variable>; ignoreRdfType?: boolean; resourceSet?: ResourceSet },
) => Resource<IdentifierT>;

export type $ToRdfResourceValuesFunction<
  ValueT,
  ReturnT extends BlankNode | Literal | NamedNode = BlankNode | Literal | NamedNode,
> = (
  value: ValueT,
  options: {
    graph?: Exclude<Quad_Graph, Variable>;
...
function $validateArray<ItemSchemaT, ItemValueT, Readonly extends boolean>(
  ...
  return (schema: $CollectionSchema<ItemSchemaT>, valueArray: readonly ItemValueT[]): Either<Error, EitherR> => {
    if (schema.minCount !== undefined && valueArray.length < schema.minCount) {
      return Left(
        new Error(`value array has length (${valueArray.length}) less than minCount (${schema.minCount})`),
...
function $wrap_ToRdfResourceFunction<
  IdentifierT extends Resource.Identifier,
  ObjectT extends { $identifier: () => IdentifierT },
>(_toRdfResourceFunction: $_ToRdfResourceFunction<IdentifierT, ObjectT>): $ToRdfResourceFunction<ObjectT, IdentifierT> {
...
export namespace FormStruct {
  export function create(
    parameters: {
      readonly $identifier?: (() => FormStruct.Identifier) | BlankNode | NamedNode | string;
    ...
    return $sequenceRecord({
      ...
      emptyStringSetProperty: $convertToScalarSet($identityConversionFunction, true)(parameters.emptyStringSetProperty)
        .chain((value) =>
      ...
      nonEmptyStringSetProperty: $convertToScalarSet($identityConversionFunction, true)(
      ...
      optionalStringProperty: $convertToMaybe($identityConversionFunction)(parameters.optionalStringProperty).chain(
        (value) =>
          $validateMaybe($identityValidationFunction)(FormStruct.schema.properties.optionalStringProperty.type, value)
      ),
    ...
    }).map((properties) => ({ ...properties, $type: "FormStruct" as const })).map((object) =>
  ...
  export namespace Json {
    ...
    export function schema() {
      return z.object({
        ...
        "emptyStringSetProperty": z.string().array().optional().readonly().meta({ "title": "Empty string set" }),
        "nestedStructProperty": NestedStruct.Json.schema().meta({ "title": "Nested object" }),
        "nonEmptyStringSetProperty": z.string().array().nonempty().min(1).readonly().meta({
          "title": "Non-empty string set",
        }),
        "optionalStringProperty": z.string().optional().meta({ "title": "Optional string" }),
    ...
    export function uiSchema(parameters?: { scopePrefix?: string }): any {
      ...
      return {
        "elements": [
          { label: "Identifier", scope: `${scopePrefix}/properties/@id`, type: "Control" },
          {
            rule: {
              condition: { schema: { const: "FormStruct" as const }, scope: `${scopePrefix}/properties/@type` },
              effect: "HIDE",
            },
            scope: `${scopePrefix}/properties/@type`,
            type: "Control",
          },
          { label: "Empty string set", scope: `${scopePrefix}/properties/emptyStringSetProperty`, type: "Control" },
          NestedStruct.Json.uiSchema({ scopePrefix: `${scopePrefix}/properties/nestedStructProperty` }),
          {
            label: "Non-empty string set",
            scope: `${scopePrefix}/properties/nonEmptyStringSetProperty`,
            type: "Control",
          },
          { label: "Optional string", scope: `${scopePrefix}/properties/optionalStringProperty`, type: "Control" },
          { label: "Required int", scope: `${scopePrefix}/properties/requiredIntProperty`, type: "Control" },
          { label: "Required string", scope: `${scopePrefix}/properties/requiredStringProperty`, type: "Control" },
        ],
        label: "FormStruct",
        type: "Group",
      };
  ...
  export function fromJson($json: FormStruct.Json): Either<Error, FormStruct> {
    return $sequenceRecord({
      ...
      emptyStringSetProperty: Either.sequence<Error, string>(
        ($json["emptyStringSetProperty"] ?? []).map((item) => (Either.of<Error, string>(item))),
      ),
      nestedStructProperty: NestedStruct.fromJson($json["nestedStructProperty"]),
      nonEmptyStringSetProperty: Either.sequence<Error, string>(
        $json["nonEmptyStringSetProperty"].map((item) => (Either.of<Error, string>(item))),
      ),
      optionalStringProperty: Maybe.fromNullable($json["optionalStringProperty"]).map((item) =>
        (Either.of<Error, string>(item)).map(Maybe.of)
      ).orDefault(Either.of(Maybe.empty())),
      requiredIntProperty: Either.of<Error, number>($json["requiredIntProperty"]),
      requiredStringProperty: Either.of<Error, string>($json["requiredStringProperty"]),
    }).chain(create);
  }
  ...
  export const _fromRdfResource: $_FromRdfResourceFunction<FormStruct> = ($resource, _$options) => {
    return ($sequenceRecord({
      ...
      emptyStringSetProperty: $shaclPropertyFromRdf<readonly (string)[], $CollectionSchema<$StringSchema<string>>>({
        "context": _$options.context,
        "graph": _$options.graph,
        "focusResource": $resource,
        "ignoreRdfType": true,
        ...
        "typeFromRdfResourceValues": $setFromRdfResourceValues<string, $StringSchema<string>>(
          $stringFromRdfResourceValues<string>,
        ),
      }),
      nestedStructProperty: $shaclPropertyFromRdf<NestedStruct, typeof NestedStruct.schema>({
        "context": _$options.context,
        "graph": _$options.graph,
        "focusResource": $resource,
        "ignoreRdfType": true,
        ...
        "typeFromRdfResourceValues": NestedStruct.fromRdfResourceValues,
      }),
      nonEmptyStringSetProperty: $shaclPropertyFromRdf<readonly (string)[], $CollectionSchema<$StringSchema<string>>>({
        "context": _$options.context,
        "graph": _$options.graph,
        "focusResource": $resource,
        "ignoreRdfType": true,
      ...
      optionalStringProperty: $shaclPropertyFromRdf<Maybe<string>, $MaybeSchema<$StringSchema<string>>>({
        "context": _$options.context,
        "graph": _$options.graph,
        "focusResource": $resource,
        "ignoreRdfType": true,
        ...
        "typeFromRdfResourceValues": $maybeFromRdfResourceValues<string, $StringSchema<string>>(
          $stringFromRdfResourceValues<string>,
        ),
      }),
      requiredIntProperty: $shaclPropertyFromRdf<number, $NumericSchema<number>>({
        "context": _$options.context,
        "graph": _$options.graph,
        "focusResource": $resource,
        "ignoreRdfType": true,
      ...
      requiredStringProperty: $shaclPropertyFromRdf<string, $StringSchema<string>>({
        "context": _$options.context,
        "graph": _$options.graph,
        "focusResource": $resource,
        "ignoreRdfType": true,
  ...
  export const fromRdfResourceValues: $FromRdfResourceValuesFunction<FormStruct, typeof FormStruct.schema> = (
    values,
    options,
  ) =>
  ...
  export const schema = {
    properties: {
      $identifier: { kind: "Identifier", type: { kind: "Identifier" as const } },
  ...
  export function toJson(_formStruct: FormStruct): FormStruct.Json {
    return JSON.parse(JSON.stringify(
      {
        "@id": _formStruct.$identifier().termType === "BlankNode"
          ? `_:${_formStruct.$identifier().value}`
          : _formStruct.$identifier().value,
        "@type": _formStruct.$type,
        emptyStringSetProperty: _formStruct.emptyStringSetProperty.map((item) => item),
        nestedStructProperty: NestedStruct.toJson(_formStruct.nestedStructProperty),
        nonEmptyStringSetProperty: _formStruct.nonEmptyStringSetProperty.map((item) => item),
        optionalStringProperty: _formStruct.optionalStringProperty.map((item) => item).extract(),
  ...
  export const _toRdfResource: $_ToRdfResourceFunction<FormStruct.Identifier, FormStruct> = (parameters) => {
    parameters.resource.add(
      ...
      parameters.object.emptyStringSetProperty.flatMap((item) => [$literalFactory.string(item)]),
      parameters.graph,
    );
    parameters.resource.add(FormStruct.schema.properties.nestedStructProperty.path, [
    ...
    parameters.resource.add(
      ...
      parameters.object.nonEmptyStringSetProperty.flatMap((item) => [$literalFactory.string(item)]),
      parameters.graph,
    );
    parameters.resource.add(
      FormStruct.schema.properties.optionalStringProperty.path,
      parameters.object.optionalStringProperty.toList().flatMap((value) => [$literalFactory.string(value)]),
      parameters.graph,
    );
    parameters.resource.add(FormStruct.schema.properties.requiredIntProperty.path, [
      $literalFactory.number(parameters.object.requiredIntProperty, $RdfVocabularies.xsd.int),
    ], parameters.graph);
    parameters.resource.add(FormStruct.schema.properties.requiredStringProperty.path, [
...
export namespace NestedStruct {
  export function create(
    parameters: {
      readonly $identifier?: (() => NestedStruct.Identifier) | BlankNode | NamedNode | string;
      readonly requiredStringProperty: string;
    },
    ...
    return $sequenceRecord({
      $identifier: $convertToIdentifierProperty(parameters.$identifier),
      requiredStringProperty: Either.of(parameters.requiredStringProperty),
    }).map((properties) => ({ ...properties, $type: "NestedStruct" as const })).map((object) =>
      $monkeyPatchObject(object, { toJson, $toString })
    );
  }
  ...
  export namespace Json {
    ...
    export function uiSchema(parameters?: { scopePrefix?: string }): any {
      const scopePrefix = parameters?.scopePrefix ?? "#";
      return {
        "elements": [{ label: "Identifier", scope: `${scopePrefix}/properties/@id`, type: "Control" }, {
          rule: {
            condition: { schema: { const: "NestedStruct" as const }, scope: `${scopePrefix}/properties/@type` },
            effect: "HIDE",
          },
          scope: `${scopePrefix}/properties/@type`,
          type: "Control",
  ...
  export const fromRdfResourceValues: $FromRdfResourceValuesFunction<NestedStruct, typeof NestedStruct.schema> = (
    values,
    options,
  ) =>
  ...
  export function toJson(_nestedStruct: NestedStruct): NestedStruct.Json {
    return JSON.parse(
      JSON.stringify(
        {
          "@id": _nestedStruct.$identifier().termType === "BlankNode"
  ...
  export const _toRdfResource: $_ToRdfResourceFunction<NestedStruct.Identifier, NestedStruct> = (parameters) => {
    parameters.resource.add(NestedStruct.schema.properties.requiredStringProperty.path, [
      $literalFactory.string(parameters.object.requiredStringProperty),
    ], parameters.graph);
    return parameters.resource;
  };
...
export namespace $Object {
  ...
  export const fromJson = (value: $Object.Json): Either<Error, $Object> => {
    if ((value["@type"] === "FormStruct")) {
      return FormStruct.fromJson(value as FormStruct.Json).map((value) => value);
    }
    if ((value["@type"] === "NestedStruct")) {
      return NestedStruct.fromJson(value as NestedStruct.Json).map((value) => value);
    }
  ...
  export const fromRdfResource: $FromRdfResourceFunction<$Object> = (resource, options) =>
    (FormStruct.fromRdfResource(resource, { ...options, ignoreRdfType: false }) as Either<Error, $Object>).altLazy(
      () => (NestedStruct.fromRdfResource(resource, { ...options, ignoreRdfType: false }) as Either<Error, $Object>)
    );

  export const fromRdfResourceValues: $FromRdfResourceValuesFunction<$Object, typeof $Object.schema> =
    ((values, options) =>
      values.chainMap((value) => {
        const valueAsValues = value.toValues();
        return (FormStruct.fromRdfResourceValues(valueAsValues, {
          ...options,
          schema: options.schema.members["FormStruct"].type,
        }) as Either<Error, Resource.Values<$Object>>).altLazy(
          () => (NestedStruct.fromRdfResourceValues(valueAsValues, {
            ...options,
            schema: options.schema.members["NestedStruct"].type,
  ...
  export namespace Json {
    export const schema = () =>
      z.discriminatedUnion("$type", [FormStruct.Json.schema(), NestedStruct.Json.schema()]).readonly().meta({});
  ...
  export const schema = {
    ...
    members: {
      "FormStruct": { "discriminantValues": ["FormStruct"], "type": FormStruct.schema },
      "NestedStruct": { "discriminantValues": ["NestedStruct"], "type": NestedStruct.schema },
    },
    properties: {
      requiredStringProperty: {
        kind: "Shacl",
  ...
  export const toRdfResourceValues = ((value, _options): (BlankNode | NamedNode)[] => {
    if (FormStruct.isFormStruct(value)) {
      return [FormStruct.toRdfResource(value, { graph: _options.graph, resourceSet: _options.resourceSet }).identifier];
    }
    if (NestedStruct.isNestedStruct(value)) {
      return [
        NestedStruct.toRdfResource(value, { graph: _options.graph, resourceSet: _options.resourceSet }).identifier,
      ];
    }
