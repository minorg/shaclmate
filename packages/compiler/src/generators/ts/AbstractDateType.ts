...
export abstract class AbstractDateType extends AbstractPrimitiveType<Date> {
  ...
  override readonly equalsFunction = code`${conditionalOutput(
...
function ${syntheticNamePrefix}filterDate(filter: ${localSnippets.DateFilter}, value: Date) {
  ...
  override get schemaType(): Code {
    return code`${conditionalOutput(`${syntheticNamePrefix}DateSchema`, code`type ${syntheticNamePrefix}DateSchema = ${this.schemaObject};`)}`;
  }
  ...
  override get sparqlWherePatternsFunction(): Code {
    return code`${conditionalOutput(
      ...
      code`\
const ${syntheticNamePrefix}dateSparqlWherePatterns: ${sharedSnippets.SparqlWherePatternsFunction}<${this.filterType}, ${this.schemaType}> =
  ...
  protected override get schemaTypeObject() {
    return {
      ...
      kind: code`"DateTimeType" | "DateType"`,
      "in?": "readonly Date[]",
    };
  }
  ...
  override toRdfExpression({
    ...
    return code`[${sharedImports.dataFactory}.literal(${this.toIsoStringExpression(variables)}, ${rdfjsTermExpression(this.xsdDatatype)})]`;
  }
  ...
  >[0]): ReturnType<AbstractPrimitiveType<Date>["fromRdfExpressionChain"]> {
    ...
    if (this.primitiveIn.length > 0) {
      ...
      fromRdfResourceValueExpression = `${fromRdfResourceValueExpression}.chain(primitiveValue => { ${this.primitiveIn.map((value) => `if (primitiveValue.getTime() === ${value.getTime()}) { return purify.Either.of${eitherTypeParameters}(primitiveValue); }`).join(" ")} return ${sharedImports.Left}${eitherTypeParameters}(new ${sharedImports.Resource}.MistypedTermValueError(${{ actualValue: "value.toTerm()", expectedValueType: this.name, focusResource: variables.resource, predicate: variables.predicate }})); })`;
    }
